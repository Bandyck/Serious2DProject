...
void SetProblem(HWND hWnd, const int problem, const Problem_Data & pd, HBITMAP & hImage, BITMAP & Image, HWND hBt[], HMENU hBtMenu[]);
// >> : JSON
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
		...
		enum Magic
		{
			Geo1 = 0, Geo2, Geo3, Geo4, Geo5, Geo6,
			Ari11, Ari12, Ari21, Ari22, Ari31, Ari32, Ari41, Ari42, Ari51, Ari52, Ari61, Ari62
		};
		enum Choice
		{
		...
		ProblemData.Magic = 0;		ProblemData.ProblemNo = "";		ProblemData.AnswerNumber = 0;
		for (int i = 0; i<4; i++)
		...
		case Geo1:
		{
			SetProblem(hWnd, Geo1, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Geo2:
		{
			SetProblem(hWnd, Geo2, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Geo3:
		{
			SetProblem(hWnd, Geo3, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Geo4:
		{
			SetProblem(hWnd, Geo4, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Geo5:
		{
			SetProblem(hWnd, Geo5, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
			break;
		case Geo6:
		{
			SetProblem(hWnd, Geo6, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari11:
		{
			SetProblem(hWnd, Ari11, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
			break;
		case Ari12:
		{
			SetProblem(hWnd, Ari12, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari21:
		{
			SetProblem(hWnd, Ari21, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari22:
		{
			SetProblem(hWnd, Ari22, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari31:
		{
			SetProblem(hWnd, Ari31, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari32:
		{
			SetProblem(hWnd, Ari32, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari41:
		{
			SetProblem(hWnd, Ari41, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari42:
		{
			SetProblem(hWnd, Ari42, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari51:
		{
			SetProblem(hWnd, Ari51, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari52:
		{
			SetProblem(hWnd, Ari52, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari61:
		{
			SetProblem(hWnd, Ari61, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Ari62:
		{
			SetProblem(hWnd, Ari62, ProblemData, hProblemImage, bitProblem, hBtChoice, hBtMenuChoice);
		}
		break;
		case Cho1:
		{
...
void SetProblem(HWND hWnd, const int problem, const Problem_Data & pd, HBITMAP & hImage, BITMAP & Image, HWND hBt[], HMENU hBtMenu[])
{
	wstring result = SetProblemPath(pd);
	LPCWSTR ProblemPath = result.c_str();
	hProblemImage = (HBITMAP)LoadImage(NULL, ProblemPath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	GetObject(hProblemImage, sizeof(BITMAP), &bitProblem);
	for (int i = 0; i < 4; i++)
	{
		string str = pd.Choice[i];
		wstring stemp = s2ws(str);
		LPCWSTR result = stemp.c_str();
		hBtMenu[i] = (HMENU)(i + 100);
		hBt[i] = CreateWindow(TEXT("BUTTON"), result, WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
			992 + 136 * i, 704, 136, 64, hWnd, hBtMenu[i], hInst, NULL);
	}
	InvalidateRgn(hWnd, NULL, TRUE);
}